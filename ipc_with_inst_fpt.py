...
detailed_stats_base = {
    ...
    "L0_access_cycles": 0.0,
    "L1_access_cycles": 0.0,
    "L2_access_cycles": 0.0,
    "L3_access_cycles": 0.0,
    ...
    "L0_access_avg_cycle": 0,
    "L1_access_avg_cycle": 0,
    "L2_access_avg_cycle": 0,
    "L3_access_avg_cycle": 0,
}
...
def get_page_walk_latency_radix(pgwk_str: str) -> float:
    ...
    for idx, stat in enumerate(pgwk_str.split(",")):
        if stat in LATENCY:
            ...
            assert idx < 4
            # per_layer_latency[idx][stat] += freq
            # stats["L{}_access_times".format(idx)] += freq
            # stats["L{}_access_cycles".format(idx)] += freq * cur_latency
        else:
            print("Invalid stat: {}".format(stat))
    return cur_latency

def get_page_walk_latency_radix_stats(pgwk_str: str, freq, stats) -> float:
    if len(pgwk_str) == 0:
        return 0
    ...
    for idx, stat in enumerate(pgwk_str.split(",")):
        level_latency = 0.0
        if stat in LATENCY:
            level_latency += LATENCY[stat]

            pwc_extra = 0
            if stat == "PWC":
                pwc_extra = LATENCY["PWC"] * (2 - idx)

            level_latency += pwc_extra
            cur_latency += level_latency

            assert idx < 4
            # per_layer_latency[idx][stat] += freq
            stats["L{}_access_times".format(idx)] += freq
            stats["L{}_access_cycles".format(idx)] += freq * level_latency
        else:
            print("Invalid stat: {}".format(stat))
    return cur_latency
...
def get_page_walk_latency_fpt(pgwk_str: str, freq, stats) -> float:
    if len(pgwk_str) == 0:
        return 0

    cur_latency = 0.0
    accesses = pgwk_str.split(",")
    # fake FPT for now.
    ...
    if FPT_FLAVOR == "L4L3_L2L1":
        ...
        if accesses[1] == "ZERO" or accesses[2] == "PWC":
            # L3 PWC hit, L2 must hit
            step1 = LATENCY["PWC"]
        else:
            step1 = LATENCY[accesses[1]] + LATENCY["PWC"]
        cur_latency += step1
        
        if (stats is not None):
            stats["L1_access_times"] += freq
            stats["L1_access_cycles"] += freq * step1
        
        
        if accesses[3] == "ZERO":
            # huge page
            step2 = LATENCY[accesses[2]]
        else:
            step2 = LATENCY[accesses[3]]
        cur_latency += step2
        
        if (stats is not None):
            stats["L3_access_times"] += freq
            stats["L3_access_cycles"] += freq * step2
        
    elif FPT_FLAVOR == "L3L2":
        cur_latency = LATENCY[accesses[0]] + LATENCY[accesses[2]] + LATENCY[accesses[3]]

    # print(pgwk_str, cur_latency)
    # for idx, stat in enumerate(pgwk_str.split(",")):
    #     if stat in LATENCY:
    #         cur_latency += LATENCY[stat]
...
def parseOneLine(line: str, stats: dict[str, float], arch: str):
    ...
    if arch == "radix":
        ...
        pgwk_latency = get_page_walk_latency_fpt(pgwk_str, freq, None)
        ...
        if nmem == 0:
            ...
            if icache_hit == 1:
                ...
                if ltb_hit == 1:
                    ...
                    if arch == "radix":
                        get_page_walk_latency_radix_stats(pgwk_str, freq, stats)
                    elif arch == 'fpt':
                        get_page_walk_latency_fpt(pgwk_str, freq, stats)
...
def post_parsing_process(stats: dict[str, float]):
    ...
    stats["L0_access_avg_cycle"] =  stats["L0_access_cycles"] / stats["L0_access_times"] if stats["L0_access_times"] > 0 else 0
    stats["L1_access_avg_cycle"] =  stats["L1_access_cycles"] / stats["L1_access_times"] if stats["L1_access_times"] > 0 else 0
    stats["L2_access_avg_cycle"] =  stats["L2_access_cycles"] / stats["L2_access_times"] if stats["L2_access_times"] > 0 else 0 
    stats["L3_access_avg_cycle"] =  stats["L3_access_cycles"] / stats["L3_access_times"] if stats["L3_access_times"] > 0 else 0
...
def calc_ipc_speedup(radix_stats, ecpt_stats, stage):
    ...
    speedups = pd.concat([pgwalk_speedup, ipc_speedup, e2e_speedup, e2e_reduction], axis=1)
    speedups.columns = ['pgwalk_speedup', 'ipc_speedup', 'e2e_speedup', 'e2e_reduction']
    ...
    speedups = pd.concat([all_ones, all_ones, all_ones, all_ones], axis=1)
    speedups.columns = ['pgwalk_speedup', 'ipc_speedup', 'e2e_speedup', 'e2e_reduction']
    radix_stats = pd.concat([speedups, radix_stats], axis=1)
    
    file_name = f'{FPT_FLAVOR}/ipc_stats_{stage}.csv'
    ...
    return radix_stats, ecpt_stats, file_name


def calc_running_ipc():
    print(LATENCY)
    
    RADIX_FOLDER = '/data1/collect_trace_fast/radix/'
    ECPT_FOLDR = '/hdd/collect_trace_fast/ecpt_before_submit/'
    stage = 'running'
    arch = 'radix'
    ...
    bench_infos = [(b,              'frontier', f'{RADIX_FOLDER}/radix_never_{b}_dyna_asplos_smalltlb_config_realpwc_with_ifetch_2024-07-24_05-34-53.log') for b in benchs]
    ...
    # bench_infos += [('postgres',     'CSL',      '/disk/bak1/siyuan/postgres64G_sequential_load/radix_always_run_postgres64G_sequential_load.bin.dyna_asplos_smalltlb_config_realpwc.log')]    
    
    radix_stats = process_from_list(bench_infos, arch, stage)
    arch = 'fpt'
    
    # fpt_bench_info = bench_infos.copy()

    fpt_bench_info = [
        ('graphbig_bfs', 'frontier', f'/data1/fpt_report/fpt_L4L3_L2L1/running/fpt_L4L3_L2L1_never_graphbig_bfs_running_walk_log.bin.dyna_asplos_smalltlb_config_realpwc.log'),
        ('sysbench', 'frontier', f'/data1/fpt_report/fpt_L4L3_L2L1/running/fpt_L4L3_L2L1_never_sysbench_running_walk_log.dyna_asplos_smalltlb_config_realpwc.log'),
        ('gups', 'frontier', f'/data1/fpt_report/fpt_L4L3_L2L1/running/fpt_L4L3_L2L1_never_gups_running_walk_log.bin.dyna_asplos_smalltlb_config_realpwc.log'),
    ]
    ...
    ecpt_stats = process_from_list(fpt_bench_info, arch, stage)
...
def calc_loading_end_ipc():
    ...
    RADIX_FOLDER = '/hdd/alan_loading_phase_end/loading_end_phase'
    ECPT_FOLDR = RADIX_FOLDER
    stage = 'loading_end'
    arch = 'radix'
    ...
    fpt_stats = process_from_list(fpt_bench_info, arch, stage)
    
    return calc_ipc_speedup(radix_stats, fpt_stats, stage)
...
def get_unified(runnig_perf, loading_perf, running_inst_ratio, loading_inst_ratio, arch):
    ...
    break_down = False
    if arch == "radix" or arch == "fpt":
        break_down = True
        unified_L0_access_cycles = get_unified_cycles(
            runnig_perf,
            loading_perf,
            selected_running_inst_ratio,
            selected_loading_inst_ratio,
        ...
        unified_L1_access_cycles = get_unified_cycles(
            runnig_perf,
            loading_perf,
            selected_running_inst_ratio,
            selected_loading_inst_ratio,
        ...
        unified_L2_access_cycles = get_unified_cycles(
            runnig_perf,
            loading_perf,
            selected_running_inst_ratio,
            selected_loading_inst_ratio,
        ...
        unified_L3_access_cycles = get_unified_cycles(
            runnig_perf,
            loading_perf,
            selected_running_inst_ratio,
            selected_loading_inst_ratio,
    ...
    unified_df = pd.DataFrame({
        ...
        'L0_access_avg_cycles': unified_L0_access_cycles / unified_n_page_walk if break_down else 0,
        'L1_access_avg_cycles': unified_L1_access_cycles / unified_n_page_walk if break_down else 0,
        'L2_access_avg_cycles': unified_L2_access_cycles / unified_n_page_walk if break_down else 0,
        'L3_access_avg_cycles': unified_L3_access_cycles / unified_n_page_walk if break_down else 0,
        
    })

    file_name = f'{FPT_FLAVOR}/ipc_unified_{arch}.csv'
    with open(file_name, 'w') as f:
        f.write(f'{arch}\n')
        f.write('running inst ratio\n')
        selected_running_inst_ratio.to_csv(f)

        f.write('unified results\n')
        unified_df.to_csv(f)

    print('save to ', file_name)

    return unified_df, file_name

def save_all_files_into_csv(all_files, out_dest):
...
if __name__ == "__main__":
    ...
    radix_unified, unified_radix_out_file = get_unified(radix_running, radix_loading_end, running_inst_ratio, loading_inst_ratio, 'radix')
    ecpt_unified, unified_comp_out_file = get_unified(ecpt_running, ecpt_loading_end, running_inst_ratio, loading_inst_ratio, 'fpt')
    ...
    final_speedup_file = f'{FPT_FLAVOR}/speedup_radix_vs_fpt.csv'
    with open(final_speedup_file, 'w') as f:
        speedups.to_csv(f)
    
    print(speedups)
    print('IPC speedup: ', speedups.mean())
    print('save to ', final_speedup_file)
    
    all_files = [
        running_out_file,
        loading_end_out_file,
        unified_radix_out_file,
        unified_comp_out_file,
        final_speedup_file
    ]
    
    save_all_files_into_csv(all_files, f'{FPT_FLAVOR}/all_results.csv')